#!/usr/bin/env python3
"""
Real Experiment with SONAR Data - PyTorch Implementation
Implements and evaluates four advanced methods on real nursing activity data
"""
...
def run_ablation_study(train_loader, val_loader, test_loader, num_classes, input_size, epochs=200):
    ...
    if ablation_results:
        ...
        # Save as JSON
        with open('../results/ablation_study/ablation_results.json', 'w') as f:
        ...
        ablation_df.to_csv('../results/ablation_study/ablation_summary.csv')
        
        print(f"\n✅ Ablation study results saved:")
        print(f"   - JSON: '../results/ablation_study/ablation_results.json'")
        print(f"   - CSV: '../results/ablation_study/ablation_summary.csv'")
        print(f"   - Visualization: '../results/ablation_study/ablation_visualization.png'")
...
def create_ablation_visualization(ablation_results):
    ...
    # Save the plot
    os.makedirs('../results/ablation_study', exist_ok=True)
    plt.savefig('../results/ablation_study/ablation_visualization.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    try:
        ...
        if results:
            ...
            for model_name, result in results.items():
                ...
                # Save individual training history
                history_file = f"../results/training_histories/{model_name.replace(' ', '_')}_history.json"
                with open(history_file, 'w') as f:
                    json.dump(result['training_history'], f, indent=2)
                
                # Save classification report
                report_file = f"../results/{model_name.replace(' ', '_')}_classification_report.json"
                with open(report_file, 'w') as f:
                    json.dump(result['classification_report'], f, indent=2)
            ...
            summary_df.to_csv('../results/pytorch_experimental_results.csv')
            
            # Save detailed results as JSON
            detailed_results_file = '../results/detailed_experimental_results.json'
            ...
            create_results_visualization(results, label_encoder.classes_)
            create_training_curves(results)
            create_confusion_matrices(results, label_encoder.classes_)
            create_roc_curves(results, label_encoder.classes_)
            
            # Show improvement analysis
            if 'Baseline CNN-LSTM' in results and len(results) > 1:
                best_model = max(results.keys(), key=lambda k: results[k]['test_metrics']['accuracy'])
                baseline_acc = results['Baseline CNN-LSTM']['test_metrics']['accuracy']
            ...
            print(f"\n✅ Results saved to:")
            print(f"   - Summary: '../results/pytorch_experimental_results.csv'")
            print(f"   - Detailed: '../results/detailed_experimental_results.json'")
            print(f"   - Training histories: '../results/training_histories/'")
            print(f"   - Confusion matrices: '../results/confusion_matrices/'")
            print(f"   - ROC curves: '../results/roc_curves/'")
            print(f"   - All visualizations: '../results/*.png'")
...
def create_results_visualization(results, class_names):
    """Create comprehensive results visualization"""
    plt.figure(figsize=(20, 15))
    
    models_list = list(results.keys())
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FECA57', '#FF9FF3'][:len(models_list)]
    
    # Extract metrics for visualization
    train_acc = [results[model]['train_metrics']['accuracy'] for model in models_list]
    val_acc = [results[model]['val_metrics']['accuracy'] for model in models_list]
    test_acc = [results[model]['test_metrics']['accuracy'] for model in models_list]
    
    train_f1 = [results[model]['train_metrics']['f1_score'] for model in models_list]
    val_f1 = [results[model]['val_metrics']['f1_score'] for model in models_list]
    test_f1 = [results[model]['test_metrics']['f1_score'] for model in models_list]
    
    training_times = [results[model]['training_time'] for model in models_list]
    epochs_trained = [results[model]['epochs_trained'] for model in models_list]
    
    # 1. Accuracy comparison across datasets
    plt.subplot(3, 3, 1)
    x = np.arange(len(models_list))
    width = 0.25
    plt.bar(x - width, train_acc, width, label='Train', color='lightcoral', alpha=0.8)
    plt.bar(x, val_acc, width, label='Validation', color='lightblue', alpha=0.8)
    plt.bar(x + width, test_acc, width, label='Test', color='lightgreen', alpha=0.8)
    plt.ylabel('Accuracy')
    plt.title('Accuracy Comparison Across Datasets', fontweight='bold')
    plt.xticks(x, [model.split()[0] for model in models_list], rotation=45)
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # 2. F1-Score comparison
    plt.subplot(3, 3, 2)
    plt.bar(x - width, train_f1, width, label='Train', color='lightcoral', alpha=0.8)
    plt.bar(x, val_f1, width, label='Validation', color='lightblue', alpha=0.8)
    plt.bar(x + width, test_f1, width, label='Test', color='lightgreen', alpha=0.8)
    plt.ylabel('F1-Score')
    plt.title('F1-Score Comparison Across Datasets', fontweight='bold')
    plt.xticks(x, [model.split()[0] for model in models_list], rotation=45)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.subplot(3, 3, 3)
    scatter = plt.scatter(training_times, test_acc, s=[e*10 for e in epochs_trained], 
                         c=range(len(models_list)), cmap='viridis', alpha=0.7)
    for i, model in enumerate(models_list):
        plt.annotate(model.split()[0], (training_times[i], test_acc[i]), 
                    xytext=(5, 5), textcoords='offset points', fontsize=9)
    plt.xlabel('Training Time (s)')
    plt.ylabel('Test Accuracy')
    plt.title('Training Efficiency (bubble size = epochs)', fontweight='bold')
    plt.grid(True, alpha=0.3)
    
    # 4. Precision comparison
    plt.subplot(3, 3, 4)
    train_prec = [results[model]['train_metrics']['precision'] for model in models_list]
    val_prec = [results[model]['val_metrics']['precision'] for model in models_list]
    test_prec = [results[model]['test_metrics']['precision'] for model in models_list]
    plt.bar(x - width, train_prec, width, label='Train', color='lightcoral', alpha=0.8)
    plt.bar(x, val_prec, width, label='Validation', color='lightblue', alpha=0.8)
    plt.bar(x + width, test_prec, width, label='Test', color='lightgreen', alpha=0.8)
    ...
    # 5. Recall comparison
    plt.subplot(3, 3, 5)
    train_rec = [results[model]['train_metrics']['recall'] for model in models_list]
    val_rec = [results[model]['val_metrics']['recall'] for model in models_list]
    test_rec = [results[model]['test_metrics']['recall'] for model in models_list]
    plt.bar(x - width, train_rec, width, label='Train', color='lightcoral', alpha=0.8)
    plt.bar(x, val_rec, width, label='Validation', color='lightblue', alpha=0.8)
    plt.bar(x + width, test_rec, width, label='Test', color='lightgreen', alpha=0.8)
    plt.ylabel('Recall')
    ...
    # 6. Training time vs epochs
    plt.subplot(3, 3, 6)
    plt.bar(models_list, training_times, color=colors[:len(models_list)], alpha=0.7)
    for i, (model, time, epochs) in enumerate(zip(models_list, training_times, epochs_trained)):
        plt.text(i, time + max(training_times)*0.01, f'{epochs}e', 
                ha='center', va='bottom', fontweight='bold')
    plt.ylabel('Training Time (s)')
    plt.title('Training Time (epochs shown)', fontweight='bold')
    plt.xticks(rotation=45, ha='right')
    plt.grid(True, alpha=0.3)
    
    # 7. Overfitting analysis (Train vs Test gap)
    plt.subplot(3, 3, 7)
    overfit_acc = np.array(train_acc) - np.array(test_acc)
    overfit_f1 = np.array(train_f1) - np.array(test_f1)
    plt.bar(x - 0.2, overfit_acc, 0.4, label='Accuracy Gap', alpha=0.7)
    plt.bar(x + 0.2, overfit_f1, 0.4, label='F1-Score Gap', alpha=0.7)
    ...
    # 8. Radar chart for comprehensive comparison
    plt.subplot(3, 3, 8)
    categories = ['Test Acc', 'Test F1', 'Test Prec', 'Test Rec', 'Val Acc']
    angles = np.linspace(0, 2 * np.pi, len(categories), endpoint=False).tolist()
    angles += angles[:1]
    
    for i, model in enumerate(models_list):
        values = [
            results[model]['test_metrics']['accuracy'],
            results[model]['test_metrics']['f1_score'],
            results[model]['test_metrics']['precision'],
            results[model]['test_metrics']['recall'],
            results[model]['val_metrics']['accuracy']
        ]
        values += values[:1]
        plt.plot(angles, values, 'o-', linewidth=2, label=model.split()[0], 
    ...
    for model in models_list:
        row = [
            model.split()[0],
            f"{results[model]['test_metrics']['accuracy']:.3f}",
            f"{results[model]['test_metrics']['f1_score']:.3f}",
            f"{results[model]['training_time']:.1f}s",
            f"{results[model]['epochs_trained']}"
        ]
        table_data.append(row)
    
    table = plt.table(cellText=table_data,
                     colLabels=['Model', 'Test Acc', 'Test F1', 'Time', 'Epochs'],
                     cellLoc='center',
                     loc='center')
    table.auto_set_font_size(False)
    table.set_fontsize(9)
    table.scale(1.2, 1.5)
    plt.title('Summary Table', fontweight='bold', pad=20)
...
def create_training_curves(results):
    ...
    for i, (model_name, result) in enumerate(results.items()):
        ...
        ax1.plot(epochs, history['train_acc'], '--', color=colors[i], alpha=0.7, 
                label=f'{model_name.split()[0]} (Train)')
        ax1.plot(epochs, history['val_acc'], '-', color=colors[i], linewidth=2,
                label=f'{model_name.split()[0]} (Val)')
    ...
    for i, (model_name, result) in enumerate(results.items()):
        history = result['training_history']
        epochs = history['epochs']
        ax2.plot(epochs, history['train_loss'], '--', color=colors[i], alpha=0.7,
                label=f'{model_name.split()[0]} (Train)')
        ax2.plot(epochs, history['val_loss'], '-', color=colors[i], linewidth=2,
                label=f'{model_name.split()[0]} (Val)')
    ...
    for i, (model_name, result) in enumerate(results.items()):
        history = result['training_history']
        epochs = history['epochs']
        ax3.plot(epochs, history['train_f1'], '--', color=colors[i], alpha=0.7,
                label=f'{model_name.split()[0]} (Train)')
        ax3.plot(epochs, history['val_f1'], '-', color=colors[i], linewidth=2,
                label=f'{model_name.split()[0]} (Val)')
    ...
    for i, (model_name, result) in enumerate(results.items()):
        history = result['training_history']
        epochs = history['epochs']
        ax4.plot(epochs, history['learning_rate'], color=colors[i], linewidth=2,
...
def create_confusion_matrices(results, class_names):
    ...
    fig.suptitle('Confusion Matrices (Test Set)', fontsize=18, fontweight='bold')
    
    axes = axes.flatten()
    
    for i, (model_name, result) in enumerate(results.items()):
        if i >= 4:  # Maximum 4 models in 2x2 grid
            break
        ...
        # Create heatmap without numbers to avoid overlapping
        ...
        plt.tight_layout()
        filename = f"../results/confusion_matrices/{model_name.replace(' ', '_')}_confusion_matrix.png"
...
def create_roc_curves(results, class_names):
    """Create ROC curves for all models"""
    print("  Generating ROC curves...")
    
    # Create individual ROC curves for each model
    for model_name, result in results.items():
        if 'predictions' not in result['test_metrics'] or 'targets' not in result['test_metrics']:
            continue
            
        y_true = result['test_metrics']['targets']
        y_pred = result['test_metrics']['predictions']
        
        if len(y_true) == 0:
            continue
        
        # For multi-class ROC, we need probability predictions
        # Since we don't have probabilities, we'll create a simplified version
        ...
        for i in range(n_classes):
            if i < y_true_bin.shape[1] and i < y_pred_bin.shape[1]:
                # Simple binary classification for each class
                y_true_class = y_true_bin[:, i]
                y_score_class = y_pred_bin[:, i]
                
                # If we have both positive and negative samples
        ...
        plt.tight_layout()
        filename = f"../results/roc_curves/{model_name.replace(' ', '_')}_roc_curves.png"
        os.makedirs("../results/roc_curves", exist_ok=True)
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        plt.close()
    
    # Create combined ROC curves comparison
    ...
    for idx, (model_name, result) in enumerate(results.items()):
        ...
        # Calculate average accuracy as a proxy for overall performance
        accuracy = accuracy_score(y_true, y_pred)
        
        # Create a simplified ROC-like curve based on accuracy
        # This is a simplified approach since we don't have probability predictions
        plt.plot([0, 1-accuracy, 1], [0, accuracy, 1], 
                color=model_colors[idx % len(model_colors)], lw=3,
                label=f'{model_name.split()[0]} (Acc = {accuracy:.3f})', 
    ...
    plt.xlabel('False Positive Rate', fontsize=12)
    plt.ylabel('True Positive Rate', fontsize=12)
    plt.title('Model Performance Comparison (ROC-style)', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('../results/roc_curves/models_comparison_roc.png', dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    try:
        ...
        CONFIG = {
            'min_samples_per_class': 100,    # Minimum samples per class for inclusion
            'use_all_classes': True,        # Use all available classes regardless of sample count
            'include_ablation': False       # Disable ablation study to save time
        }
